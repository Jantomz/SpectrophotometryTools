...
class manipulator():
    ...
    def __init__(self, folderPath, calibrationFolderPath=None, csvDatasetName=None):
        ...
        if calibrationFolderPath is not None:
            ...
            self.load_normalized_data(folderPath)
    ...
    def load_normalized_data(self, folderPath):
        """
        Loads and processes the calibrated data.
        ...
        for version in range(1, 12):
            for multiplier in variation:
                for path in pathList:
                    ...
                    data = data.transpose()
                    data.columns = data.iloc[0]
                    data.index = [path.split('/')[-1].split('.')[0] + '_' + str(multiplier) + '_' + "_version" + str(version) + "_" + str(i-1) for i in range(1, len(data)+1)]
                    data = data[1:]

                    threshold = 0.2

                    # Averaging out the values a bit more to smooth it out
                    data = data.apply(lambda row:  row.rolling(window=5, center=True, min_periods=1).mean(), axis=1)

                    # Calculate the standard deviation of each row
                    row_std = data.std(axis=1)

                    # Add the standard deviation as a new column in the DataFrame
                    data[2] = row_std

                    # Apply rolling average to rows with standard deviation less than threshold
                    low_std_rows = data[data[2] < threshold].index
                    data.loc[low_std_rows] = data.loc[low_std_rows].apply(lambda row: row.rolling(window=16, center=True, min_periods=1).mean(), axis=1)

                    # Divide every data point in the row by the mean value of the row
                    data = data.div(data.mean(axis=1), axis=0)
                    
                    # Trimming the data to only include the first 800 columns
                    data = data.loc[:, data.columns.map(lambda x: int(x) <= 700)]

                    # Multiplier for the variance
                    individual_variation = [random.uniform(0.99, 1.01) for _ in range(len(data.columns))]

                    data = data.mul(multiplier)

                    # Multiply each column in data by the corresponding value in individual_variation
                    data = data.mul(individual_variation)
